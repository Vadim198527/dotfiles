...
;;   )
  ...
  (use-package corfu
    ...
    (corfu-auto-prefix 3)	       ;; Показывать после ввода 1 символа
    (corfu-auto-delay 0.0)       ;; Без задержки перед показом
    (corfu-quit-at-boundary nil) ;; Не закрывать на границах
    (corfu-quit-no-match nil) ;; Не закрывать, даже если нет совпадений
    (corfu-preview-current t) ;; Предпросмотр текущего кандидата
    ...
    (corfu-history-mode))          ;; Сохраняет историю выбора
